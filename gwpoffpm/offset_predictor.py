...
class OffsetPredictor:
    ...
    def __call__(self, x):
        ...
        # test the input size
        assert x.shape[1] == self.input_dim, f"Wrong size of the input. {x.shape[1]} != {self.input_dim}."
        
        # test whether leg_to_move is one-hot encoded
        assert len(np.unique(x[:, -5:])) == 2, f"'leg_to_move' attribute is not one-hot encoded. Values: {x[:, -5:]}."
        assert np.all(np.sum(x[:, -5:], axis=1) == 1), f"'leg_to_move' attribute is not one-hot encoded. Values: {x[:, -5:]}."
...
if __name__ == "__main__":
    ...
    offset_predictor = OffsetPredictor(model_path="model_v0.pth")
    ...
    offset = offset_predictor(sample_data)
    print(offset)
